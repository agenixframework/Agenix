...
public class TestReportersTest
{
    ...
    private Mock<IAsyncTestCase> _testCaseMock;

    [SetUp]
    public void BeforeMethodSetup()
    {
        _testCaseMock = new Mock<IAsyncTestCase>();
        _fixture = new TestReporters();
    }

    [Test]
    public async Task OnStartWithoutAutoClear()
    {
        var testResults = _fixture.GetTestResults();

        _fixture.SetAutoClear(false);
        await _fixture.OnStart();
    ...
    [Test]
    public async Task OnStartWithAutoClear()
    {
        var testResults = _fixture.GetTestResults();
    ...
    [Test]
    public async Task OnFinishSuccessGeneratesReports()
    {
        await VerifyGenerateReport(() => _fixture.OnFinishSuccess());
    }

    [Test]
    public async Task OnFinishFailureGeneratesReports()
    {
        var cause = new Mock<Exception>();

        await VerifyGenerateReport(() => _fixture.OnFinishFailure(cause.Object));

        cause.VerifyNoOtherCalls();
    }

    private async Task VerifyGenerateReport(Func<Task> invocation)
    {
        var testReporter1 = new Mock<IAsyncTestReporter>();
        _fixture.AddTestReporter(testReporter1.Object);

        var testReporter2 = new Mock<IAsyncTestReporter>();
    ...
    [Test]
    public async Task OnTestFinishIgnoresNullTestResult()
    {
