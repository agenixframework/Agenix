...
public class RandomStringFunctionTest : AbstractNUnitSetUp
{
    ...
    public void TestRandomStringFunction()
    {
        ...
        Assert.That(_function.Execute(parameters, Context).Length, Is.EqualTo(3));
    ...
    public void TestWithNumbers()
    {
        ...
        Assert.That(_function.Execute(parameters, Context).Length, Is.EqualTo(10));

        parameters = ["10", RandomStringFunction.Lowercase, "true"];

        Assert.That(_function.Execute(parameters, Context).Length, Is.EqualTo(10));

        parameters = ["10", RandomStringFunction.Mixed, "true"];

        Assert.That(_function.Execute(parameters, Context).Length, Is.EqualTo(10));

        parameters = ["10", "UNKNOWN", "true"];
