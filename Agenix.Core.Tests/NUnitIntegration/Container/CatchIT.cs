...
#pragma warning restore CS0649 // Field is never assigned to, and will always have its default value
    ...
    [Test]
    public async Task CatchAction()
    {
        await _gherkin.When(CatchException().When(Fail("Fail!")));

        await _gherkin.When(CatchException().Exception(typeof(AgenixSystemException)).When(Fail("Fail!")));

        await _gherkin.When(CatchException()
