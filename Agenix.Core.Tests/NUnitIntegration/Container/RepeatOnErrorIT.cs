...
#pragma warning restore CS0649 // Field is never assigned to, and will always have its default value
    ...
    [Test]
    public async Task RepeatOnErrorContainer()
    {
        await _gherkin.Given(CreateVariable("message", "Hello Test Framework!"));

        await _gherkin.When(RepeatOnError().Name("Repeat just 1").Until("i = 5").Index("i")
            .Actions(Echo("${i}. Attempt: ${message}")));

        await _gherkin.When(RepeatOnError().Name("Repeat just 1 with the sleep of 500 ms").Until("i = 5").AutoSleep(500)
            .Index("i").Actions(Echo("${i}. Attempt: ${message}")));

        await _gherkin.When(Assert().Exception(typeof(AgenixSystemException))
            .When(
