...
/// </summary>
public interface IAsyncTestActionContainer : IAsyncTestAction
{
    /**
     * Sets the embedded test actions to execute within this container.
     */
    IAsyncTestActionContainer SetActions(List<IAsyncTestAction> actions);

    /**
     * Get the embedded test actions within this container.
     */
    List<IAsyncTestAction> GetActions();
     ...
     */
    IAsyncTestActionContainer AddTestActions(params IAsyncTestAction[] action);

    /**
     * Adds a test action to the nested action list.
     */
    IAsyncTestActionContainer AddTestAction(IAsyncTestAction action);

    /**
     * Returns the index in the action chain for the provided action instance.
     */
    int GetActionIndex(IAsyncTestAction action);

    /**
    * Sets the current active action executed.
    */
    void SetActiveAction(IAsyncTestAction action);

    /**
    * Sets the last action that has been executed.
    */
    void SetExecutedAction(IAsyncTestAction action);

    /**
    * Get the action that was executed most recently.
    */
    IAsyncTestAction? GetActiveAction();

    /**
    * Gets all nested actions that have been executed in the container.
    */
    List<IAsyncTestAction> GetExecutedActions();

    /**
    * Get the test action with given index in list.
    */
    IAsyncTestAction GetTestAction(int index);
}
