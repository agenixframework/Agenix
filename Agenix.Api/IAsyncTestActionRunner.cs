...
/// /
public interface IAsyncTestActionRunner
{
    /// Runs the provided test action.
    /// @param action The test action to be executed.
    /// @return The executed test action.
    /// /
    Task<T> Run<T>(T action) where T : IAsyncTestAction;

    /// Runs the provided test action.
    /// @param action The test action to be executed.
    /// @return The executed test action.
    Task<T> Run<T>(Func<T> action) where T : IAsyncTestAction;

    /// Runs a given test action.
    /// @param action The test action to run.
    /// @return The result of the test action.
    /// /
    Task<T> Run<T>(IAsyncTestActionBuilder<T> builder) where T : IAsyncTestAction;
    ...
    Task<IAsyncTestActionBuilder<IAsyncTestAction>> ApplyBehavior(IAsyncTestBehavior behavior);
    ...
    Task<IAsyncTestActionBuilder<IAsyncTestAction>> ApplyBehavior(AsyncTestBehavior behavior);
}
