...
*/
public interface IAsyncTestCase : IAsyncTestActionContainer, INamed, IAsyncDescribed
{
    ...
    /// /
    Task Start(TestContext context);
    ...
    Task ExecuteAction(IAsyncTestAction action, TestContext context);

    /// Executes cleanup tasks after the test case, regardless of the test result.
    /// <param name="context">The context in which the test case is executed.</param>
    Task Finish(TestContext context);
    ...
    List<IAsyncTestActionBuilder<IAsyncTestAction>> GetActionBuilders();
