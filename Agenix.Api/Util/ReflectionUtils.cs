...
public sealed class ReflectionUtils
{
    ...
    ///     <para>Searches with BindingFlags</para>
    ///     <para>
    ///         When dealing with interface methods, you probably want to 'normalize' method references by calling
    ///         <see cref="MapInterfaceMethodToImplementationIfNecessary" />.
    ///     </para>
    /// </remarks>
    /// <param name="targetType">
    ...
    public static MethodInfo GetGenericMethod(Type type, string methodName, Type[] typeArguments, Type[] parameterTypes)
    {
        ...
        if (typeArguments == null)
        {
            ...
            methodInfo = type.GetMethod(methodName,
                BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance, null, parameterTypes, null);
        }
        else
        {
            // Generic Method
    ...
    /// </exception>
    public static MethodInfo GetMethodByArgumentValues<T>(IEnumerable<T> methods, object[] argValues)
        where T : MethodBase
    {
            ...
            if (isMatch)
            {
                ...
                else
                {
                    throw new AmbiguousMatchException(
                        string.Format("Ambiguous match for {0} '{1}' for the specified number and types of arguments.",
                            methodTypeName,
                            m.Name));
                }
            }
    ...
    public static CustomAttributeBuilder CreateCustomAttribute(CustomAttributeData attributeData)
    {
        ...
        for (var i = 0; i < attributeData.ConstructorArguments.Count; i++)
        {
            ...
            parameterValues[i] =
                ConvertConstructorArgsToObjectArrayIfNecessary(attributeData.ConstructorArguments[i].Value);
        }
        ...
        foreach (var namedArgument in attributeData.NamedArguments)
        {
            ...
            for (var j = 0; j < attributeProperties.Length; j++)
            {
                if (attributeProperties[j].Name == namedArgument.MemberInfo.Name)
                {
                    ...
                    namedParameterValues.Add(
                        ConvertConstructorArgsToObjectArrayIfNecessary(namedArgument.TypedValue.Value));
                    break;
                }
    ...
    public static void SetInstanceFieldValue(object obj, string fieldName, object fieldValue)
    {
        ...
        if (f != null)
        {
            if (f.FieldType != fieldValue.GetType())
            {
                throw new ArgumentException(
                    string.Format(
                        "fieldValue for fieldName '{0}' of object type '{1}' must be of type '{2}' but was of type '{3}'",
                        fieldName, obj.GetType(), f.FieldType, fieldValue.GetType()), "fieldValue");
            }
    ...
    private static MemberwiseCopyHandler GetImpl(Type type)
    {
        ...
        lock (s_handlerCache)
        {
            ...
            var callback = () =>
            {
                var dm = new DynamicMethod(type.FullName + ".ShallowCopy", null,
                    new[] { typeof(object), typeof(object) }, type.Module, true);
                ...
                callback();
#else
            SecurityCritical.ExecutePrivileged(new PermissionSet(PermissionState.Unrestricted), callback);
#endif
    ...
    {
        private readonly ArrayList constructorArgs;
        private readonly List<PropertyInfo> namedProperties;
        private readonly List<object> propertyValues;
        ...
        public void AddPropertyValue(string name, object value)
        {
            ...
            {
                throw new ArgumentException(
                    string.Format("The property '{0}' does no exist in the attribute '{1}'.", name, type));
            }
        ...
        {
            var caArray = (object[])constructorArgs.ToArray(typeof(object));
            var ci = type.GetConstructor(GetTypes(caArray));
            ...
            {
                var npArray = namedProperties.ToArray();
                var pvArray = propertyValues.ToArray();
