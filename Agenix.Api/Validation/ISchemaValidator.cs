...
public interface ISchemaValidator<in T> where T : ISchemaValidationContext
{
    ...
    ///     Logger.
    /// </summary>
    private static readonly ILog Log = LogManager.GetLogger(typeof(ISchemaValidator<T>).Name);
    ...
    /// </summary>
    /// <returns>A dictionary containing the instantiated schema validators,
    /// keyed by their identifiers.</returns>
    static IDictionary<string, ISchemaValidator<T>> Lookup()
    {
        ...
        if (!Log.IsDebugEnabled) return resolvedSchemas;
        {
            foreach (var kvp in resolvedSchemas) Log.Debug($"Found schema validator '{kvp.Key}' as {kvp.Value.GetType()}");
        }
