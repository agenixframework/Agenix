...
public class AsyncMessageListeners : IAsyncMessageListenerAware
{
    private readonly List<IAsyncMessageListener> _messageListeners = [];
    ...
    public IList<IAsyncMessageListener> MessageListeners => _messageListeners.AsReadOnly();
    ...
    public virtual async Task OnInboundMessage(IMessage? message, TestContext context)
    {
        if (message == null)
        {
            return;
        ...
        {
            await listener.OnInboundMessage(message, context);
        }
    }
    ...
    public virtual async Task OnOutboundMessage(IMessage? message, TestContext context)
    {
        if (message == null)
        {
            return;
        ...
        foreach (var listener in _messageListeners)
        {
            await listener.OnOutboundMessage(message, context);
        }
    }
