...
/// /
public class AsyncTestActionListeners : IAsyncTestActionListenerAware
{
    /** List of test action listeners **/
    private readonly List<IAsyncTestActionListener> _testActionListeners = [];

    /// Gets the TestActionListeners.
    /// @return A read-only list of test action listeners.
    /// /
    public IReadOnlyList<IAsyncTestActionListener> TestActionListeners => _testActionListeners.AsReadOnly();
    ...
    public async Task OnTestActionFinish(IAsyncTestCase testCase, IAsyncTestAction testAction)
    {
        foreach (var listener in _testActionListeners)
        {
            await listener.OnTestActionFinish(testCase, testAction);
        }
    }
    ...
    public async Task OnTestActionSkipped(IAsyncTestCase testCase, IAsyncTestAction testAction)
    {
        foreach (var listener in _testActionListeners)
        {
            await listener.OnTestActionSkipped(testCase, testAction);
        }
    }
    ...
    public async Task OnTestActionStart(IAsyncTestCase testCase, IAsyncTestAction testAction)
    {
        foreach (var listener in _testActionListeners)
        {
            await listener.OnTestActionStart(testCase, testAction);
        }
    }
}
