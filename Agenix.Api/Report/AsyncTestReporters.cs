...
/// Implements the IAsyncTestListener, IAsyncTestSuiteListener, and IAsyncTestReporterAware interfaces to manage the test lifecycle and reports.
public class AsyncTestReporters : IAsyncTestListener, IAsyncTestSuiteListener, IAsyncTestReporterAware
{
    /// Collection of asynchronous test reporters used to handle test reporting and lifecycle events.
    private readonly List<IAsyncTestReporter> _testReporters = [];

    /// Stores accumulated test results to provide an overview of test execution outcomes.
    /// Utilized for compiling and managing test summary data in various reporting operations.
    /// /
    private TestResults _testResults = new();

    /// Indicates whether test results should be automatically cleared at the start of a new test or test suite execution.
    public static bool AutoClear
    {
        get => AgenixSettings.ReportAutoClear();
    ...
    /// It allows for any necessary cleanup or state update related to the successful completion of a test.
