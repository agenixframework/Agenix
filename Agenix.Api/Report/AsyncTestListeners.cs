...
/// </summary>
public class AsyncTestListeners : IAsyncTestListenerAware
{
    /** Thread-safe collection of test listeners **/
    private readonly ConcurrentDictionary<IAsyncTestListener, byte> _testListeners = new();
    ...
    public bool RemoveTestListener(IAsyncTestListener listener)
    {
    ...
    public int RemoveTestListenersOfType<T>() where T : IAsyncTestListener
    {
    ...
    public List<T> GetTestListenersOfType<T>() where T : IAsyncTestListener
    {
    ...
    public async Task OnTestFailure(IAsyncTestCase test, Exception cause)
    {
        ...
        foreach (var listener in listeners)
        {
            try
            {
                await listener.OnTestFailure(test, cause);
            }
            catch (Exception ex)
            {
    ...
    public async Task OnTestFinish(IAsyncTestCase test)
    {
        ...
        foreach (var listener in listeners)
        {
            try
            {
                await listener.OnTestFinish(test);
            }
            catch (Exception ex)
            {
    ...
    public async Task OnTestSkipped(IAsyncTestCase test)
    {
        var listeners = _testListeners.Keys.ToList(); // Snapshot for thread safety
        foreach (var listener in listeners)
        {
            try
            {
                await listener.OnTestSkipped(test);
            }
            catch (Exception ex)
            {
                // Log or handle listener exceptions
    ...
    public async Task OnTestStart(IAsyncTestCase test)
    {
        var listeners = _testListeners.Keys.ToList(); // Snapshot for thread safety
        foreach (var listener in listeners)
        {
            try
            {
                await listener.OnTestStart(test);
            }
            catch (Exception ex)
            {
                // Log or handle listener exceptions
    ...
    public async Task OnTestSuccess(IAsyncTestCase test)
    {
        var listeners = _testListeners.Keys.ToList(); // Snapshot for thread safety
        foreach (var listener in listeners)
        {
            try
            {
                await listener.OnTestSuccess(test);
            }
            catch (Exception ex)
            {
                // Log or handle listener exceptions
    ...
    public bool ContainsTestListener(IAsyncTestListener listener)
    {
