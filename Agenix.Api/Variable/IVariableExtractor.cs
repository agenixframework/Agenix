...
public interface IVariableExtractor : IMessageProcessor
{
    ...
    /// <summary>
    ///     Lazy-initialized type resolver that dynamically identifies and locates custom variable extractors
    ...
    /// <summary>
    ///     Lazy-initialized cache for individual extractor builder lookups to avoid repeated resolution attempts.
    /// </summary>
    private static readonly Lazy<ConcurrentDictionary<string, object>> ExtractorBuilderCache =
        ...
        where TB : IBuilder
    {
        ...
        var cachedResult = ExtractorBuilderCache.Value.GetOrAdd(cacheKey, key =>
        {
            try
            {
                var instance = TypeResolver.Value.Resolve<TB>(extractor);
                return Optional<TB>.Of(instance);
            }
            catch (AgenixSystemException)
            {
                Log.LogWarning(
                    "Failed to resolve variable extractor from resource '{ResourcePath}/{Extractor}'",
                    ResourcePath, extractor);
                return Optional<TB>.Empty;
            }
        });

        return (Optional<TB>)cachedResult;
    }
