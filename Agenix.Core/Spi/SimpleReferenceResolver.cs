...
public class SimpleReferenceResolver : IReferenceResolver
{
    ...
    /// <exception cref="CoreSystemException">Thrown when no object of the specified type is found in the object store.</exception>
    public T Resolve<T>()
    {
        try
        {
        ...
        {
            throw new CoreSystemException($"Unable to find bean reference for type '{typeof(T)}'");
        }
    }
    ...
    public object Resolve(string name)
    {
        if (!_objectStore.TryGetValue(name, out var value))
            throw new CoreSystemException($"Unable to find bean reference for name '{name}'");
