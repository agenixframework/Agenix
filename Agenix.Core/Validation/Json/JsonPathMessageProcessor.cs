...
public class JsonPathMessageProcessor : AbstractMessageProcessor
{
    ...
    public new void ProcessMessage(IMessage message, TestContext context)
    {
        ...
        {
            throw new CoreSystemException("Failed to parse JSON text", ex);
        }
    }
    ...
    /// <summary>
    ///     Determines if the given message type is supported by this processor.
    /// </summary>
    ...
    public new bool SupportsMessageType(string messageType)
    {
        return MessageType.JSON.ToString().Equals(messageType, StringComparison.OrdinalIgnoreCase);
    }
