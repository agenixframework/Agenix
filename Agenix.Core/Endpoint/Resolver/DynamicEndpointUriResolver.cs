...
public class DynamicEndpointUriResolver : IEndpointUriResolver
{
    ...
    /// <param name="uri">The base endpoint URI to append the request path to.</param> <param name="headers">A dictionary containing the headers which may include a request path.</param> <return>The endpoint URI with the appended request path, if provided in headers; otherwise, the original URI.</return>
    /// /
    private string AppendRequestPath(string uri, IDictionary<string, object> headers)
    {
        if (!headers.TryGetValue(IEndpointUriResolver.RequestPathHeaderName, out object value))
        {
            return uri;
        }
    ...
    /// <param name="uri">The original URI to which query parameters are to be appended.</param> <param name="headers">The collection of message headers which may include query parameters.</param> <return>Returns the URI with appended query parameters if available; otherwise, returns the original URI.</return>
    /// /
    ...
    /// Sets the default fallback endpoint URI. <param name="newDefaultEndpointUri">the new default URI to set</param>
    /// /
