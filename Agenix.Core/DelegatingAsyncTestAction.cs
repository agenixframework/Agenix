...
public class DelegatingAsyncTestAction(TestActionAsync testAction) : IAsyncTestAction
{
    /// <summary>
    ///     Executes the test action using the provided test context and optional cancellation token.
    /// </summary>
    /// <param name="context">The test context to use for execution.</param>
    /// <param name="cancellationToken">The token to monitor for cancellation requests.</param>
    /// <returns>A task representing the asynchronous operation of the test action execution.</returns>
    public Task ExecuteAsync(TestContext context, CancellationToken cancellationToken = default)
    {
        return testAction(context);
    }
}
