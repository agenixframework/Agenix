...
{
    private List<IMessageProcessor> _messageProcessors = new();
    ...
    public void AddMessageProcessor(IMessageProcessor processor)
    {
        if (processor is IScoped scopedProcessor && !scopedProcessor.IsGlobalScope())
            throw new CoreSystemException("Unable to add non-global scoped processor to global message processors - " +
