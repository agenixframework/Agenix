...
namespace Agenix.Core.Message
{
    ...
    public class DefaultMessage : IMessage
    {
        ...
        public DefaultMessage() : this((object)null)
        {
        }
        ...
        public DefaultMessage(object payload, IDictionary<string, object> headers)
        {
            _payload = payload;
            _headers.Merge(headers);

            DictionaryExtensions.SafeDictionaryAdd(_headers, MessageHeaders.Id, Guid.NewGuid().ToString());
            DictionaryExtensions.SafeDictionaryAdd(_headers, MessageHeaders.Timestamp, Environment.TickCount);
        }
        ...
        {
            return (TX)Convert.ChangeType(Payload, typeof(TX));
        }
