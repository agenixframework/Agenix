...
///     Generate method.
/// </summary>
public abstract class AbstractAsyncTestReporter : IAsyncTestReporter
{
    /// <summary>
    ///     Logger.
    /// </summary>
    private static readonly ILogger Log = LogManager.GetLogger(typeof(AbstractAsyncTestReporter));
    ...
    public async Task GenerateReport(TestResults testResults)
    {
        try
        {
            await Generate(testResults);
        }
        catch (Exception e)
        {
            if (IgnoreErrors)
    ...
    public abstract Task Generate(TestResults testResults);
}
