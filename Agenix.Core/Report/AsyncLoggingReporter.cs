...
/// </summary>
public class AsyncLoggingReporter : AbstractAsyncTestReporter, IAsyncMessageListener, IAsyncTestSuiteListener,
    IAsyncTestListener, IAsyncTestActionListener
{
    ...
    private readonly ILogger _enabledInboundMessageLogger;

    /// The standard logger used when the reporter is enabled
    private readonly ILogger _enabledLog;

    /// The logger used for outbound messages when the reporter is enabled.
    private readonly ILogger _enabledOutboundMessageLogger;

    /// Inbound message logger
    private ILogger _inboundMessageLogger = LogManager.GetLogger("Logger.Message_IN");

    private bool _isInitialized;

    /// Logger
    /// /
    private ILogger _logger = LogManager.GetLogger(typeof(AsyncLoggingReporter));

    /// Outbound message logger
    private ILogger _outboundMessageLogger = LogManager.GetLogger("Logger.Message_OUT");

    /// <summary>
    ///     Represents an asynchronous logging reporter which handles various lifecycle events
    ///     and generates logs for inbound and outbound messages, test actions, and test outcomes.
    /// </summary>
    public AsyncLoggingReporter()
    {
        _enabledInboundMessageLogger = _inboundMessageLogger;
        _enabledOutboundMessageLogger = _outboundMessageLogger;
        _enabledLog = _logger;
    }
    ...
    /// </param>
    public Task OnInboundMessage(IMessage message, TestContext context)
    {
    ...
    public Task OnOutboundMessage(IMessage message, TestContext context)
    {
    ...
    public Task OnTestActionStart(IAsyncTestCase testCase, IAsyncTestAction testAction)
    {
        if (_logger.IsEnabled(LogLevel.Debug))
        {
            ...
            if (testAction is IAsyncTestActionContainer container)
            {
                Debug($"TEST ACTION CONTAINER with {container.GetActionCount()} embedded actions");
            }

            if (testAction is not IAsyncDescribed described || string.IsNullOrWhiteSpace(described.GetDescription()))
            {
                return Task.CompletedTask;
            }
    ...
    public Task OnTestActionFinish(IAsyncTestCase testCase, IAsyncTestAction testAction)
    {
        if (_logger.IsEnabled(LogLevel.Debug))
        {
            NewLine();
    ...
    public Task OnTestActionSkipped(IAsyncTestCase testCase, IAsyncTestAction testAction)
    {
        if (_logger.IsEnabled(LogLevel.Debug))
        {
            NewLine();
            ...
            Debug($"TEST ACTION {testAction.Name} SKIPPED");
        }
    ...
    public Task OnTestFailure(IAsyncTestCase test, Exception cause)
    {
        ...
        Error($"TEST FAILED {test.Name} <{test.GetNamespaceName()}>{duration} Nested exception is: ", cause);
    ...
    public Task OnFinishFailure(Exception cause)
    {
        NewLine();
    ...
    ///     is null.
    /// </returns>
    private static string FormatDurationString(IAsyncTestCase test)
    {
    ...
    public override Task Generate(TestResults testResults)
    {
        NewLine();
    ...
    public void SetEnabled(bool enabled)
    {
        ...
        {
            _logger = _enabledLog;
            _inboundMessageLogger = _enabledInboundMessageLogger;
            _outboundMessageLogger = _enabledOutboundMessageLogger;
        }
        else
        {
            _logger = NoOpLogger;
