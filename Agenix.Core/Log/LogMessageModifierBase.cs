...
namespace Agenix.Core.Log
{
    ...
    public abstract class LogMessageModifierBase : ILogModifier
    {
        ...
        public Dictionary<string, object> MaskHeaders(IMessage message)
        {
            ...
            foreach (KeyValuePair<string, object> entry in message.GetHeaders())
            {
                if (entry.Value == null)
                {
                ...
                string keyValuePair = $"{entry.Key}={entry.Value}";
        ...
        List<string> MaskHeaderData(IMessage message)
        {
            ...
            return message.GetHeaderData().Select(x => Mask(x)).ToList();
        }
    }
}