...
public class DefaultTestActionBuilder(TestActionAsync action)
    : AbstractAsyncTestActionBuilder<IAsyncTestAction, DefaultTestActionBuilder>
{
    ...
    /// /
    public static DefaultTestActionBuilder Action(TestActionAsync action)
    {
    ...
    public override AbstractTestActionAsync Build()
    {
        AbstractTestActionAsync testAction = new ConcreteTestAction(action);
    ...
    private sealed class ConcreteTestAction(TestActionAsync delegateInstance) : AbstractAsyncTestAction
    {
        public override async Task DoExecuteAsync(TestContext context)
        {
            if (delegateInstance != null)
            {
                await delegateInstance.Invoke(context);
            }
        }
    }
}
