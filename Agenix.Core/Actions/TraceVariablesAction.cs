...
/// </summary>
public class TraceVariablesAction(TraceVariablesAction.Builder builder) : AbstractTestActionAsync("trace", builder)
{
    ...
    public override Task DoExecute(TestContext context, CancellationToken cancellationToken = default)
    {
        ...
        else
        {
            ...
            using IDisposable it1 = it;
        }

        while (it.MoveNext())
        {
            var key = it.Current;
            var value = context.GetVariable(key ?? throw new AgenixSystemException("The key is null"));

            Log.LogInformation("Variable {MaskedKeyAndValue}", context.LogModifier.Mask(key + " = " + value));
        }
    ...
    public class Builder : AbstractAsyncTestActionBuilder<IAsyncTestAction, dynamic>
    {
        ...
        /// <summary>
        ///     Builds and returns an instance of <see cref="TraceVariablesAction" />.
        /// </summary>
        /// <returns>An instance of <see cref="TraceVariablesAction" /> initialized with the current builder's configuration.</returns>
