...
namespace Agenix.Validation.Json.Json
{
    ...
    public class JsonPathSegmentVariableExtractor : SegmentVariableExtractorRegistry.AbstractSegmentVariableExtractor
    {
        ...
        /// </summary>
        /// <param name="testContext">Represents the context of the current test execution, providing access to necessary services and state.</param>
        ...
        /// <returns>True if the object is null, a valid JSON string, and the specified JSONPath expression is valid; otherwise, false.</returns>
        public override bool CanExtract(TestContext testContext, object? obj, VariableExpressionSegmentMatcher matcher)
        {
            return obj == null || (obj is string jsonPath && IsJsonPredicate.Instance.Test(jsonPath) && JsonPathMessageValidationContext.IsJsonPathExpression(matcher.SegmentExpression));
        }

        /// <summary>
        /// Extracts a value from a given object based on a JSONPath expression defined in the matcher.
        /// </summary>
        /// <param name="testContext">Represents the context of the current test execution, providing access to various services and state.</param>
        /// <param name="obj">The input object from which the value is to be extracted. If null, the result will also be null.</param>
        /// <param name="matcher">Contains the JSONPath expression used to identify and extract the desired portion from the object.</param>
        /// <returns>The extracted value as an object, based on the JSONPath expression provided in the matcher. If the object is null, returns null.</returns>
        protected override object DoExtractValue(TestContext testContext, object? obj, VariableExpressionSegmentMatcher matcher)
        {
        ...
        private static object ExtractJsonPath(string json, string segmentExpression)
        {
            ...
            {
                throw new AgenixSystemException($"Unable to extract jsonPath from segmentExpression {segmentExpression}", e);
            }
        }
    }
}