...
/// <summary>
///     Message validator evaluates a set of JSONPath expressions on the message payload and checks that values are as expected.
/// </summary>
    ...
    IMessageValidator<IValidationContext>
{
        ...
        JsonPathMessageValidationContext validationContext)
    {
        ...
        try
        {
            ...
            foreach (var (key, value) in validationContext.JsonPathExpressions)
            {
                ...
                Log.DebugFormat("Validating element: {0}='{1}': OK", jsonPathExpression, expectedValue);
            }
    ...
    /// Combines multiple matching validation contexts into a single context if necessary, updating other related contexts accordingly.
    /// </summary>
    ...
    /// </param>
    /// <returns>
    /// A <see cref="JsonPathMessageValidationContext"/> instance representing the consolidated validation context if multiple matching contexts exist,
