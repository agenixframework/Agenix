...
public class JsonSchemaFilter
{
    ...
    ///     Logger.
    /// </summary>
    private static readonly ILog Log = LogManager.GetLogger(nameof(JsonSchemaFilter));
    ...
    /// <param name="schemaRepositories">The repositories containing available schemas to be filtered</param> <param name="jsonMessageValidationContext">The context that contains configuration for message validation</param> <param name="referenceResolver">The reference resolver used for retrieving bean references during lookup</param> <return>A list of filtered `SimpleJsonSchema` objects that match the validation configuration</return>
    /// /
        ...
        IReferenceResolver referenceResolver)
    {
        ...
        return IsSchemaSpecified(jsonMessageValidationContext) ? GetSchemaFromContext(jsonMessageValidationContext, referenceResolver) : MergeRepositories(schemaRepositories);
    }
        ...
        IReferenceResolver referenceResolver)
    {
        ...
        {
            Log.Debug($"Found specified schema: \"{jsonMessageValidationContext.Schema}\".");
        }
    ...
    private static List<SimpleJsonSchema> FilterByRepositoryName(List<JsonSchemaRepository> schemaRepositories,
        ISchemaValidationContext jsonMessageValidationContext)
    {
        foreach (var jsonSchemaRepository in schemaRepositories.Where(jsonSchemaRepository => string.Equals(jsonSchemaRepository.Name, jsonMessageValidationContext.SchemaRepository,
                     StringComparison.Ordinal)))
        {
            if (Log.IsDebugEnabled)
            {
                Log.Debug($"Found specified schema-repository: \"{jsonMessageValidationContext.SchemaRepository}\".");
            }
            return jsonSchemaRepository.Schemas;
        }

        throw new AgenixSystemException($"Could not find the specified schema repository: \"{jsonMessageValidationContext.SchemaRepository}\".");
    }

    /// Merges the schemas from the provided list of JSON schema repositories into a single list of SimpleJsonSchema.
    /// <param name="schemaRepositories">The list of JSON schema repositories containing schemas to be merged</param> <return>A consolidated list of SimpleJsonSchema from all provided repositories</return>
    /// /
    ...
    /// <param name="context">The context containing schema-related information.</param> <return>True if the schema is specified in the context; otherwise, false.</return>
    /// /
    ...
    /// <param name="context">The validation context that contains schema repository-related configuration.</param> <return><c>true</c> if a schema repository is specified; otherwise, <c>false</c>.</return>
    /// /
