...
public sealed record ConfigurationSingleton<T> where T : class, new()
{
    ...
    {
        return ConfigurationCache.TypeConfigurations.GetOrAdd(typeof(T),
            _ => new ConfigurationSettings
            {
    ...
    public static void Configure(ConfSettings settings)
    {
        ...
        {
            currentSettings.ConfigurationName = settings.ConfigurationName;
        }

        if (settings.ConfigurationDirectory != null)
        {
            currentSettings.ConfigurationDirectory = settings.ConfigurationDirectory;
        }

        if (settings.Format.HasValue)
        {
            currentSettings.Format = settings.Format.Value;
        }

        if (settings.EnvironmentFileSupport.HasValue)
        {
            currentSettings.EnvironmentFileSupport = settings.EnvironmentFileSupport.Value;
        }

        if (settings.EnvironmentFileDirectory != null)
        {
            currentSettings.EnvironmentFileDirectory = settings.EnvironmentFileDirectory;
        }

        if (settings.EnvironmentFileName != null)
        {
            currentSettings.EnvironmentFileName = settings.EnvironmentFileName;
        }

        if (settings.CachingEnabled.HasValue)
        {
            currentSettings.CachingEnabled = settings.CachingEnabled.Value;
        }

        if (settings.DefaultEnvironment != null)
        {
            currentSettings.DefaultEnvironment = settings.DefaultEnvironment;
        }
    }
