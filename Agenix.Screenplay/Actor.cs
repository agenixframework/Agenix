...
public class Actor : IPerformsTasks
{
    ...
    private IDictionary<string, object> _notepad = new Dictionary<string, object>();
    private IDictionary<Type, IAbility> _abilities = new Dictionary<Type, IAbility>();
    ...
    public T? GetAbilityThatExtends<T>() where T : class
    {
        ...
        return (from entry in _abilities 
            let abilityType = entry.Key where typeof(T).IsAssignableFrom(abilityType) select entry.Value as T).FirstOrDefault();
    }
    ...
    public string NameOrPronoun => (_preferredPronoun != null) ? _preferredPronoun : _name;
