...
{
    private readonly IAsyncTestAction _action = Mock.Of<IAsyncTestAction>();

    [Test]
    public async Task TestRepeatHamcrestConditionExpression()
    {
        ...
        await repeatUntilTrue.ExecuteAsync(Context);
        ...
        Mock.Get(_action).Verify(a => a.ExecuteAsync(Context, CancellationToken.None), Times.Exactly(4));
    }
}
