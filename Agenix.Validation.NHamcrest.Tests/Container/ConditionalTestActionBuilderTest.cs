...
{
    [Test]
    public async Task TestConditionalBuilderHamcrestConditionExpression()
    {
        var builder = new DefaultTestCaseRunner(Context);
        builder.SetVariable("var", 5);
        builder.SetVariable("noExecution", "true");

        await builder.Run(Conditional().When(AssertThat("${var}", Is.EqualTo("5")).AsCondition())
            .Actions(Echo("${var}"), CreateVariable("execution", "true")));

        await builder.Run(Conditional().When(AssertThat("${var}", Is.LessThan("5")).AsCondition())
